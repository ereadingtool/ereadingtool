...
tagWord model parent_msg section_number instance original_token =
    let
        id =
            String.join "-" [ "section", toString section_number, "instance", toString instance, original_token ]

        token =
            String.toLower original_token
    in
    case token == " " of
        True ->
        ...
        False ->
            let
                ...
                merging_word =
                    Text.Translations.Model.mergingWord model word_instance
            in
            Html.node "span"
...
tagSection model msg section =
    ...
    div [ id ("section-" ++ toString section.order), class "section" ]
        (Text.Section.Words.Tag.tagWordsAndToVDOM
            ...
            (isPartOfCompoundWord model section_number)
...
view_edit model parent_msg word_instance =
    let
        editing_word =
            Text.Translations.Model.editingWordInstance model word_instance
    in
    div
        [ class "edit_overlay"
...
view_btns model parent_msg word_instance =
    let
        ...
        instance_count =
            Text.Translations.Model.instanceCount model section_number normalized_word
    in
...
view_make_compound_text_word model parent_msg word_instance =
    let
        merge_state =
            Text.Translations.Model.mergeState model word_instance
...
view_add_grammemes model msg word_instance =
    let
        ...
        grammeme_value =
            Text.Translations.Model.editingGrammemeValue model word_instance
    in
    div [ class "add" ]
